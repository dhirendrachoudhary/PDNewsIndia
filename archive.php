...
<?php
	...
	$post_count = 1;
	$grid_class = '';
	$cols = mom_option('cat_grid_cols') != false ? mom_option('cat_grid_cols') : 2;
...
?>
		...
		<?php } // end of archives ?>
            ...
            <div class="posts-grid clearfix <?php echo esc_attr('cols-'.$cols)?>">
                <?php 
                if (mom_option('cat_hide_posts_from_child_cats') == 1) {
	                if (is_category()) {
	                	$current_cat = intval( get_query_var('cat') );
	                	query_posts(array('category__in' => $current_cat, 'paged' => $paged));
	                }
            	}
			...
			?>
                ...
                <?php  endif; ?>
            <?php }// end if layout ?>
	    <?php if ($layout != 't') { mom_pagination($wp_query->max_num_pages); } ?>
            <?php wp_reset_query(); ?>
            </div> <!--main column-->
