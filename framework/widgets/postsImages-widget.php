...
$output = get_transient('mom-pimages-widget-'.$this->id);
if ($orderby == 'random') {
	$output = false;
}
if ($output == false) {
...
?>
		...
		?>
			...
			<?php if (mom_post_image() != false) { ?>
				 <li><a href="<?php the_permalink(); ?>" class="simptip-position-top simptip-movable" data-tooltip="<?php the_title(); ?>"><?php mom_post_image_full('small-wide', 'small-wide-hd'); ?></a></li>
			<?php } ?>
...
<?php
    ...
    ob_end_clean();
    set_transient('mom-pimages-widget-'.$this->id, $output, 60*60*24);
}
	...
	function update( $new_instance, $old_instance ) {
		...
		delete_transient('mom-pimages-widget-'.$this->id);
		return $instance;
	}
