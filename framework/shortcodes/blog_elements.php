...
?>
<div <?php post_class('base-box blog-post default-blog-post bp-full-img bp-horizontal-share'); ?>>
    <div class="bp-entry">
		<?php if (mom_post_image() != false) { ?>
            <div class="post-img">
                <a href="<?php the_permalink(); ?>">
					<?php echo mom_post_image_full('large'); ?>
                </a>
                <span class="post-format-icon"></span>
            </div> <!--img-->
	    <?php } ?>
...
<?php
		...
		$image_size = 'news_box_big';
		if (mom_option('site_width') == 'wide') {
			$image_size = 'big-wide-img';
		}
?>
    <div class="mom-grid-item <?php echo $grid_class; ?>">
    <div <?php post_class("base-box blog-post default-blog-post bp-full-img bp-horizontal-share"); ?>>
    <div class="bp-entry">
		<?php if (mom_post_image() != false) { ?>
            <div class="post-img">
                <a href="<?php the_permalink(); ?>">
                	<?php echo mom_post_image_full($image_size, 'big-wide-img'); ?>
                </a>
                <span class="post-format-icon"></span>
            </div> <!--img-->
	    <?php } ?>
...
?>
    <div <?php post_class("base-box blog-post default-blog-post bp-vertical-share ".$share_off); ?>>
    <div class="bp-entry">
        <div class="bp-head">
            <h2><a href="<?php the_permalink(); ?>"><?php the_title(); ?></a></h2>
            <?php mom_posts_meta('bp-meta'); ?>
	...
	<?php if (mom_post_image() != false) { ?>
            <div class="post-img">
                <a href="<?php the_permalink(); ?>">
                	<?php echo mom_post_image_full('related-posts', 'big-wide-img'); ?>
                </a>
                <span class="post-format-icon"></span>
            </div> <!--img-->
	    <?php } ?>
...
<?php foreach ($months as $month => $days ) { // iterates the moths ?>
      ...
      <div class="tl-month <?php echo $t_class; ?>">
		...
		?>
		<div class="post-img">
				<a href="<?php echo get_permalink($single->ID); ?>">
					<?php echo mom_post_image_full('small-wide', 'small-wide-hd', '',$single->ID ); ?>
				</a>
		</div>
		<?php } ?>
...
elseif ( get_query_var('page') ) { $paged = get_query_var('page'); }
		...
		<?php if ($style == 'g') { ?> <div class="posts-grid clearfix <?php echo esc_attr('cols-'.$cols)?>"> <?php } ?>
		<?php while ( $query->have_posts() ) : $query->the_post();
		if ($unique_posts) {$do_unique_posts[] = get_the_ID();}

				$grid_class = '';
				if ($style == 'g') {
					if ($post_count%$cols == '0') {
							$grid_class = 'last';
					} elseif(($post_count-1)%$cols == 0) {
							$grid_class = 'first';
					} else {
						$grid_class = '';
					}
