...
<div class="mom-reveiw-system">
    <header class="review-header">
        <h2><?php if ($title) { echo $title; } else { _e('Review Overview'); } ?></h2>
        <ul class="mr-types">
		<?php
...
<div class="review-tab bars-tab">
    ...
    <div class="review-area">
		...
		<?php foreach($criterias[0] as $criteria) {
			...
			$crbg = isset($criteria['cr_bg']) ? $criteria['cr_bg'] : '';
			if ($units == 'points') {
				$cr_score = $cr_score/10;
			}
		?>
            <div class="mom-bar">
                <div class="mb-inner" style="width:<?php echo $criteria['cr_score']; ?>%; background: <?php echo $crbg; ?>;">
			<span class="cr" style="color:<?php if (isset($criteria['cr_txt'])) { echo $criteria['cr_txt'];} ?>;"><?php echo $criteria['cr_name']; ?></span>
			<span class="mb-score" style="color: <?php echo $crbg; ?>;"><?php echo $cr_score; ?><?php echo $rs_suffix; ?></span>
                </div>
            </div>
	    <?php } ?>
    ...
    <footer class="review-footer">
        <div class="user-rate bars">
            <h3  class="mom_user_rate_title"><strong class="your_rate" style="display:none;"><?php _e('Your Rating:', 'theme'); ?></strong><strong class="user_rate"><?php _e('User Rating:', 'theme'); ?></strong></h3>
	    <div class="user-rate-bar">
            	<?php echo momizat_getPostRate($id, 'bars'); ?>
	    </div>
            <span class="total-votes">(<span class="tv-count"><?php echo $voters_count; ?></span> <?php _e('votes'); ?>)</span>
        </div>
    </footer> <!-- footer -->
    <?php } ?>
    </div> <!-- bars tab -->
<?php } elseif($rs == 'style-stars') { ?>
    ...
    <div class="review-area">
		...
		<?php
			...
			if ($txt_color == '') {
				$txt_color = isset($crbg) ? $crbg : '';
			}
		?>
                <span class="cr" style="color:<?php echo $txt_color; ?>;"><?php echo $criteria['cr_name']; ?></span>
                <div class="star-rating mom_review_score" style="color:<?php echo $crbg; ?>;"><span style="width:<?php echo $criteria['cr_score']; ?>%; color: <?php echo $crbg; ?>;"></span></div>
	    </div>
	    <?php } ?>
        </div>
    </div> <!-- review area -->
    ...
    <div class="review-area">
		...
		<div class="circle">
 <input type="text" class="rc-value" data-width="147" data-height="90" data-angleArc="200" data-angleOffset="-100" data-readOnly="true" value="<?php echo $criteria['cr_score']; ?>" data-thickness=".25" data-fgColor="<?php echo isset($crbg)? $crbg: '#78bce7'; ?>" data-bgColor="<?php echo $td_color; ?>">
                    <span class="val" style="color:<?php echo $crbg; ?>;"><?php echo $cr_score; ?><?php echo $rs_suffix; ?></span>
                </div>
		<?php
			$txt_color = isset($criteria['cr_txt']) ? $criteria['cr_txt'] : '' ;
			if ($txt_color == '') {
