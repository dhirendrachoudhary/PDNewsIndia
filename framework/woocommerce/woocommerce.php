...
/*----------------------------
    add to product
 ----------------------------*/
add_action( 'woocommerce_before_shop_loop_item', 'mom_sale_flash', 10);
...
function mom_sale_flash() {
		global $product, $post;
			if ( !$product->is_in_stock() ) { 
			 echo apply_filters( 'woocommerce_sale_flash', '<span class="onsale out-of-stock">' . __( 'out of stock', 'woocommerce' ) . '</span>', $post, $product ); 
			} else {
...
function mom_add_to_cart_button()
{
	...
	{
		$output = "<a class='mom_woo_bt button' href='".get_permalink($product->id)."'>".__('Select options','framework')."</a>";
	}
...
$defaults['delimiter'] = '<i class="sep fa-icon-double-angle-right"></i>';
if (is_rtl()) {
$defaults['delimiter'] = '<i class="sep fa-icon-double-angle-left"></i>';
}
return $defaults;
}
/* ==========================================================================
