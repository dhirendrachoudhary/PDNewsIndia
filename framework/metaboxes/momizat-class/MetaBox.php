...
add_action('admin_head', array('MomizatMB_MetaBox', '_global_head'));

add_action('admin_footer', array('MomizatMB_MetaBox', '_global_foot'));

define('MOMIZATMB_MODE_ARRAY', 'array');

define('MOMIZATMB_MODE_EXTRACT', 'extract');

define('MOMIZATMB_FIELD_HINT_TEXT', 'text');

define('MOMIZATMB_FIELD_HINT_TEXTAREA', 'textarea');

define('MOMIZATMB_FIELD_HINT_CHECKBOX', 'checkbox');

define('MOMIZATMB_FIELD_HINT_CHECKBOX_MULTI', 'checkbox_multi');

define('MOMIZATMB_FIELD_HINT_RADIO', 'radio');

define('MOMIZATMB_FIELD_HINT_SELECT', 'select');

define('MOMIZATMB_FIELD_HINT_SELECT_MULTI', 'select_multi');

// depreciated, use MOMIZATMB_FIELD_HINT_SELECT_MULTI instead
define('MOMIZATMB_FIELD_HINT_SELECT_MULTIPLE', 'select_multiple');

define('MOMIZATMB_LOCK_TOP', 'top');

define('MOMIZATMB_LOCK_BOTTOM', 'bottom');

define('MOMIZATMB_LOCK_BEFORE_POST_TITLE', 'before_post_title');

define('MOMIZATMB_LOCK_AFTER_POST_TITLE', 'after_post_title');

define('MOMIZATMB_VIEW_START_OPENED', 'opened');

define('MOMIZATMB_VIEW_START_CLOSED', 'closed');

define('MOMIZATMB_VIEW_ALWAYS_OPENED', 'always_opened');

class MomizatMB_MetaBox
{
	/**
	 ...
	 * MOMIZATMB_MODE_ARRAY (default) and MOMIZATMB_MODE_EXTRACT, this option
	 * should be used when instantiating the class.
	 *
	 * @since	1.2
	 * @access	public
	 * @var		string
	 */
	var $mode = MOMIZATMB_MODE_ARRAY;

	/**
	 * When the mode option is set to MOMIZATMB_MODE_EXTRACT, you have to take
	...
	function __construct($arr)
	{
		...
		if (is_array($arr))
		{
			...
			if ($this->lock_on_top) $this->lock = MOMIZATMB_LOCK_TOP;
			elseif ($this->lock_on_bottom) $this->lock = MOMIZATMB_LOCK_BOTTOM;
	...
	{
		if (MOMIZATMB_MODE_ARRAY == $this->mode AND $key == $this->id)
		{
	...
	function _init()
	{
		...
		if ( ! MomizatMB_MetaBox::_is_post() AND ! MomizatMB_MetaBox::_is_page()) return;
	...
	function _foot()
	{
		...
		)
		{
			...
			(function($){ /* not using jQuery ondomready, code runs right away in footer */
				...
				<?php if (MOMIZATMB_LOCK_TOP == $this->lock): ?>
				...
				<?php endif; ?>
				$('#'+id).append(mb);
				<?php elseif (MOMIZATMB_LOCK_BOTTOM == $this->lock): ?>
				<?php if ('side' == $this->context): ?>
				...
				<?php endif; ?>
				$('#'+id).append(mb);
				<?php elseif (MOMIZATMB_LOCK_BEFORE_POST_TITLE == $this->lock): ?>
				<?php if ('side' != $this->context): ?>
				...
				$('#'+id).append(mb);
				<?php endif; ?>
				<?php elseif (MOMIZATMB_LOCK_AFTER_POST_TITLE == $this->lock): ?>
				<?php if ('side' != $this->context): ?>
				...
				<?php if (MOMIZATMB_VIEW_START_OPENED == $this->view): ?>
				mb.removeClass('closed');
				<?php elseif (MOMIZATMB_VIEW_START_CLOSED == $this->view): ?>
				mb.addClass('closed');
				<?php elseif (MOMIZATMB_VIEW_ALWAYS_OPENED == $this->view): ?>
	...
	{
		if ('post' == MomizatMB_MetaBox::_is_post_or_page())
		{
			return TRUE;
		}
	...
	{
		if ('page' == MomizatMB_MetaBox::_is_post_or_page())
		{
			return TRUE;
		}
	...
	{
		$post_type = MomizatMB_MetaBox::_get_current_post_type();
	...
	public static function _get_current_post_type()
	{
		...
		if ( isset( $uri ) )
		{
			...
			{
				$post_id = MomizatMB_MetaBox::_get_post_id();
	...
	public static function _global_head()
	{
		...
		// consider giving a developer access to change this via hooks/callbacks
                    ...
                    wp_enqueue_script( 'momizatClass-metabox', get_template_directory_uri(). '/framework/metaboxes/momizat-class/js/global.js', array( 'jquery' ), '1.0', true );                    
		...
		jQuery(function($)
		{
			...
			{		
				...
				if (elem.attr('class') && elem.filter('[class*=dodelete]').length)
				{
					...
					{
                                            //review
                                                all_score -= Number(100);
                                                v = elem.parents('.wpa_group').find('input[id^="mom_slider_input"]').val();
                                               score = $('.reveiw-all-score').val() - v;
                                               $('.reveiw-all-score').val(score)
 ...
 *                Review
   ========================================================================== */
    ...
    var all_score = $('.reveiw-all-score').data('all_scores');
    var score = $('.reveiw-all-score').data('score');
    var cscore = 0;
    $('.mom_meta_slider_wrap').each(function() {
        var t = $(this);
        var id = $(this).data('id');
        var slider = t.find('#mom_meta_slider-'+id);
        var input = t.find('#mom_slider_input-'+id);
        var min = 0;
        var max = 100;
        var step = 1;
        var value = input.val();
        slider.slider({
                                    ...
                                    change: function( event, ui ) {
                                        cscore = (score - value) + Number(ui.value);
                                        //alert(cscore);
                                        score = cscore;
                                        value = ui.value;
                                        fscore = cscore/all_score*100;
                                        fscore = Math.round(fscore);
                                        $('.reveiw-all-score').val(cscore);
                                        $('.reveiw-final-score').val(fscore);
                                    }
        });
        input.keyup(function() {
			...
			$('[class*=docopy-]').on('click', function(e)
			{
				...
				var the_clone = the_group.clone(true, true).removeClass('tocopy last');
                                    ...
                                    //datefield
                                        the_clone.find('input.mom-metabox-date')
                                        .removeClass('hasDatepicker')
                                        .removeData('datepicker')
                                        .unbind()
                                        .datepicker();
                                //colors
                                the_clone.find('.mom_color_meta_wrap').empty();
                                var mmcw = the_clone.find('.mom_color_meta_wrap');
                                
                                mmcw.each(function() {
                                    var mmcw_id = $(this).data('id');
                                    var mmcw_name = $(this).data('name');
                                    $(this).append('<input class="mom_meta_color" type="text" name="'+mmcw_name+'" value="">');
                                    $(this).find('.mom_meta_color').wpColorPicker({
                                ...
                                all_score += Number(100);
                                ns = $('.reveiw-all-score').val();
                                //slider         
                                    the_clone.find('.mom_meta_slider_wrap').empty();
                                    var mmsw_id = the_clone.find('.mom_meta_slider_wrap').data('id');
                                    var mmsw_name = the_clone.find('.mom_meta_slider_wrap').data('name');
                                    the_clone.find('.mom_meta_slider_wrap').append('<div id="mom_meta_slider-'+mmsw_id+'" class="mom_meta_slider"></div><input type="text" id="mom_slider_input-'+mmsw_id+'" name="'+mmsw_name+'" value="0"><span class="crs-suffix">%</span>');
                                    var jqslider = the_clone.find('.mom_meta_slider_wrap');
                                    ...
                                    slider.slider({
                                                                ...
                                                                change: function(event, ui) {
                                                                        ...
                                                                        cscore = (ns - value) + Number(ui.value);
                                                                        //alert(cscore);
                                                                        ns = cscore;
                                                                        value = ui.value;
                                                                        fscore = cscore/all_score*100;
                                ...
                                //$(myslider).
				var the_props = ['name', 'id', 'for', 'data-id', 'data-name'];
				...
				the_group.find('*').each(function(i, elem)
				{
					for (var j = 0; j < the_props.length; j++)
					{
						...
						if (the_prop)
						{
							...
							if (matchN)
							{
								the_prop = the_prop.replace(matchN[0], (+matchN[1]+1));
	...
	function _meta($post_id = NULL, $internal = FALSE)
	{
		...
		// MOMIZATMB_MODE_ARRAY

		$meta = get_post_meta($post_id, $this->id, TRUE);

		// MOMIZATMB_MODE_EXTRACT
	...
	{
		if (!$this->in_template AND $this->mode == MOMIZATMB_MODE_EXTRACT)
		{
		...
		$hints = array
		(
			MOMIZATMB_FIELD_HINT_CHECKBOX_MULTI,
			MOMIZATMB_FIELD_HINT_SELECT_MULTI,
			MOMIZATMB_FIELD_HINT_SELECT_MULTIPLE,
		);
	...
	function _save($post_id) 
	{
		...
		MomizatMB_MetaBox::clean($new_data);
		...
		if ($this->mode == MOMIZATMB_MODE_EXTRACT)
		{
			$new_fields = array();
			...
			// keep data tidy, delete values if previously using MOMIZATMB_MODE_ARRAY
		...
		else
		{
			...
			// keep data tidy, delete values if previously using MOMIZATMB_MODE_EXTRACT
