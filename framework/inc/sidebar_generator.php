...
class sidebar_generator {
	...
	public static function admin_print_scripts(){
		...
		if (file_exists(home_url().'/wp-admin/admin-ajax.php')) {
			$file = home_url().'/wp-admin/admin-ajax.php';
		} else {
			$file = site_url().'/wp-admin/admin-ajax.php';
		}
		$nonce = wp_create_nonce( 'sidebar_generator_nonce' )
		?>
			<script type="text/javascript">
				function add_sidebar( sidebar_name )
				{
					var mysack = new sack("<?php echo $file; ?>" );    
				
				  	mysack.execute = 1;
				  	mysack.method = 'POST';
					mysack.setVar( 'nonce', '<?php echo $nonce; ?>' );
	...
	public static function add_sidebar(){
		          ...
		          if ( ! wp_verify_nonce( $nonce, 'sidebar_generator_nonce' ) ) {
            	die ( '' );
						}
		$sidebars = sidebar_generator::get_sidebars();
	...
	public static function admin_menu(){
		add_theme_page(__('Custom Sidebars','framework'), __('Custom Sidebars','framework'), 'manage_options', 'sidebar_generator.php', array('sidebar_generator','admin_page'));
	}
	...
	public static function admin_page(){
		...
		<div class="wrap">
			<h2><?php _e('Custom Sidebars','theme'); ?></h2>
			<br />
			<div class="add_sidebar">
				<a href="javascript:void(0);" onclick="return add_sidebar_link()" title="Add a sidebar"><?php _e('+ Add Sidebar', 'theme') ?></a>
			</div>
			<br />
			<table class="widefat page" id="sbg_table" style="width:600px;">
				<tr>
					<th><?php _e('Name', 'theme') ?></th>
					<th><?php _e('CSS class', 'theme') ?></th>
					<th><?php _e('Remove', 'theme') ?></th>
				</tr>
				<?php
				$sidebars = sidebar_generator::get_sidebars();
				...
				<?php
					...
					?>
					<tr>
						<td colspan="3"><?php _e('No Sidebars defined', 'theme'); ?></td>
					</tr>
					<?php
				}
				?>
